...
public class IntNullTextEdgeInputFormat extends
  ...
  public class IntNullTextEdgeReader extends
      TextEdgeReaderFromEachLineProcessed<Integer[]> {
    @Override
    protected Integer[] preprocessLine(Text line) throws IOException {
      String[] tokens = SEPARATOR.split(line.toString());
      return new Integer[]{Integer.valueOf(tokens[0]),
          Integer.valueOf(tokens[1])};
    }

    @Override
    protected IntWritable getSourceVertexId(Integer[] endpoints)
      throws IOException {
      return new IntWritable(endpoints[0]);
    }

    @Override
    protected IntWritable getTargetVertexId(Integer[] endpoints)
      throws IOException {
      return new IntWritable(endpoints[1]);
    }

    @Override
    protected NullWritable getValue(Integer[] endpoints) throws IOException {
